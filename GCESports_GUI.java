...
public class GCESports_GUI extends javax.swing.JFrame {
    ...
    */
    ...
    private void displayTeams() {
    if (!teamList.isEmpty()) {
        // 1. Extract team names into a String array
        String[] teamNames = new String[teamList.size()];
        for (int i = 0; i < teamList.size(); i++) {
            teamNames[i] = teamList.get(i).getTeamName();
        }

        // 2. Create new models with the team names
        // Note: ComboBoxes usually need their own model instance if 
        // they are used independently, or they can share if they stay in sync.
        DefaultComboBoxModel<String> model1 = new DefaultComboBoxModel<>(teamNames);
        DefaultComboBoxModel<String> model2 = new DefaultComboBoxModel<>(teamNames);

        // 3. Apply the models to the JComboBoxes
        selectTeam_JComboBox.setModel(model1);
        updateTeam_JComboBox.setModel(model2);
        
        // 4. Ensure the first item is selected and details are displayed
        selectTeam_JComboBox.setSelectedIndex(0);
        updateTeam_JComboBox.setSelectedIndex(0);
        displayTeamDetails(); 
    }
}
    ...
    public void addNewTeam() {   
        ...
        //updateTeam_JComboBox.setModel("");
        
        String teamName = teamName_JTextField.getText();
        String contactPerson = contactPerson_JTextField.getText();
        String contactPhone = contactPh_JTextField.getText();
        String contactEmail = contactEm_JTextField.getText();      
        
        Team newTeam = new Team(teamName, contactPerson, contactPhone, contactEmail);
        
        try(BufferedWriter bufferedWriter = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("teams.csv", true), "UTF-8"))) {
            // teamName, contactPerson, contactPhone, contactEmail
            bufferedWriter.write(newTeam.getTeamName() + "," + newTeam.getContactPerson() + "," + newTeam.getContactPhone() + "," + newTeam.getContactEmail());
            bufferedWriter.newLine();
        }
        // no close needed - because we are using a try with 
        // resources which will handle the close automatically i believe.
        catch(FileNotFoundException fnfe) {
            IO.println("ERROR! The File cannot be found. More information > " + fnfe.getMessage());
        }
        catch(IOException ioe) {
            IO.println("ERROR! There was a problem writing to the file. More information > " + ioe.getMessage());
        }
...
//        String[] newTeamDetails = new String[4];
//        newTeamDetails[0] = teamName;
//        newTeamDetails[1] = contactPerson;
//        newTeamDetails[2] = contactPhone;
//        newTeamDetails[3] = contactEmail;
//        
//        
//        for(String info : newTeamDetails) {
//            IO.println(info);
//        }    
